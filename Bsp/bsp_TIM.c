...
/*#define***********************************************************************/
    ...
    #define TIM1_PWM_Frequence	30000  //PWM频率30KHz
    #define TIM1_PWM_Period     5600  //PWM一个周期数
#endif
...
void TIM1_PWM_Config(void)
{
    ...
    TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCNIdleState_Reset;  //N级空闲状态下的非工作状态
...
//    TIM_BDTRInitStructure.TIM_DeadTime = 7;  //98ns
    TIM_BDTRInitStructure.TIM_DeadTime = 40;  //死区时间500ns大概
...
//    TIM1->BDTR |= (1<<15);  //使能MOE位
    ...
    TIM_TimeBaseStructure.TIM_Period = TIM1_PWM_Period - 1;              //周期
    TIM_TimeBaseStructure.TIM_Prescaler = SystemCoreClock / TIM1_PWM_Period / TIM1_PWM_Frequence - 1; //主频分频给定时器,
    ...
    TIM_OCInitStructure.TIM_OCIdleState = TIM_OCIdleState_Reset;//Stop状态时正向通道死区之后输出电平
    TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;//正向输出使能
    TIM_OCInitStructure.TIM_OCNPolarity = TIM_OCNPolarity_High;//输出极性设置
    TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCNIdleState_Reset;//Stop状态时反向通道死区之后输出电平
    TIM_OCInitStructure.TIM_OutputNState = TIM_OutputNState_Enable;//反向输出使能
 ...
 *@函数名称    PWM_DutyChange()
 *@函数功能    改变PWM的占空比
 *@入口参数    PWM_TIMx(定时器),PWM_Channel(PWM输出通道),PWM_Duat(占空比)
 *@出口参数    无
 *******************************************************************************/
